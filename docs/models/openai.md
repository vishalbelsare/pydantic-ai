...
If the model you're using is not working correctly out of the box, you can tweak various aspects of how model requests are constructed by providing your own [`ModelProfile`][pydantic_ai.profiles.ModelProfile] (for behaviors shared among all model classes) or [`OpenAIChatModelProfile`][pydantic_ai.profiles.openai.OpenAIChatModelProfile] (for behaviors specific to `OpenAIChatModel`):

```py
from pydantic_ai import Agent
from pydantic_ai.models.openai import OpenAIChatModel
from pydantic_ai.profiles import InlineDefsJsonSchemaTransformer
from pydantic_ai.profiles.openai import OpenAIChatModelProfile
from pydantic_ai.providers.openai import OpenAIProvider
...
model = OpenAIChatModel(
    ...
    profile=OpenAIChatModelProfile(
        json_schema_transformer=InlineDefsJsonSchemaTransformer,  # Supported by any model class on a plain ModelProfile
        openai_supports_strict_tool_definition=False  # Supported by OpenAIChatModel only, requires OpenAIChatModelProfile
    )
)
agent = Agent(model)
```
