...
class Foo(BaseNode):
    ...
    async def run(self, ctx: GraphRunContext) -> Bar:
        self.state.x += 1
        return Bar(self.state)


@dataclass
class Bar(BaseNode[None, int]):
    state: MyState

    async def run(self, ctx: GraphRunContext) -> End[int]:
        self.state.y += 'y'
        return End(self.state.x * 2)
...
async def test_dump_load_history(graph: Graph[None, int]):
    result, history = await graph.run(Foo(MyState(1, '')))
    assert result == snapshot(4)
    assert history == snapshot(
        [
            NodeStep(node=Foo(MyState(x=2, y='')), start_ts=IsNow(tz=timezone.utc), duration=IsFloat()),
            NodeStep(node=Bar(MyState(x=2, y='y')), start_ts=IsNow(tz=timezone.utc), duration=IsFloat()),
    ...
    assert json.loads(history_json) == snapshot(
        [
            {
                'node': {'node_id': 'Foo', 'state': {'x': 2, 'y': ''}},
                'start_ts': IsStr(regex=r'20\d\d-\d\d-\d\dT.+'),
                'duration': IsFloat(),
                'kind': 'node',
            },
            {
                'node': {'node_id': 'Bar', 'state': {'x': 2, 'y': 'y'}},
                'start_ts': IsStr(regex=r'20\d\d-\d\d-\d\dT.+'),
                'duration': IsFloat(),
                'kind': 'node',
            },
