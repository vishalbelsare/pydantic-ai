...
def test_tool_retry():
    ...
    assert result.all_messages() == snapshot(
        [
            ...
            ModelResponse(
                parts=[ToolCallPart(tool_name='my_ret', args={'x': 0}, tool_call_id=IsStr())],
                usage=RunUsage(requests=1, input_tokens=61, output_tokens=8, total_tokens=69),
                model_name='test',
                timestamp=IsNow(tz=timezone.utc),
            ),
            ModelRequest(
            ...
            ModelResponse(
                ...
                usage=RunUsage(requests=1, input_tokens=62, output_tokens=12, total_tokens=74),
                model_name='test',
                timestamp=IsNow(tz=timezone.utc),
            ),
        ]
...
def test_different_content_input(content: AudioUrl | VideoUrl | ImageUrl | BinaryContent):
    ...
    assert result.output == snapshot('custom')
    assert result.usage() == snapshot(RunUsage(requests=1, input_tokens=51, output_tokens=1, total_tokens=52))
