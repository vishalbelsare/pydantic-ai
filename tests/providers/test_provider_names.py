...
with try_import() as imports_successful:
    ...
    test_infer_provider_params = [
        ...
        ('google-vertex', GoogleProvider, 'Could not resolve project using application default credentials.'),
...
def test_infer_provider(provider: str, provider_cls: type[Provider[Any]], exception_has: str | None):
    if exception_has is not None:
        with pytest.raises((UserError, OpenAIError, ValueError), match=rf'.*{exception_has}.*'):
