...
def test_run_with_history_new():
    ...
    assert result2.usage() == snapshot(RunUsage(input_tokens=55, output_tokens=13, details=None))
    ...
    assert result3.usage() == snapshot(RunUsage(input_tokens=55, output_tokens=13, details=None))
...
def test_run_with_history_new_structured():
    ...
    assert result2.usage() == snapshot(RunUsage(input_tokens=59, output_tokens=13, details=None))
    new_msg_part_kinds = [(m.kind, [p.part_kind for p in m.parts]) for m in result2.all_messages()]
    assert new_msg_part_kinds == snapshot(
        [
            ('request', ['system-prompt', 'user-prompt']),
